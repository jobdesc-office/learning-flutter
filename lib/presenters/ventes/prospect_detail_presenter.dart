...
import '../../contracts/base/history_view_contract.dart';
...
import '../../views/ventes/prospect/prospectfiles/choosed_popup.dart';
...
class ProspectDetailPresenter extends CustomGetXController {
  ...
  late HistoryViewContract _historyViewContract;
  set historyViewContract(HistoryViewContract historyViewContract) {
    _historyViewContract = historyViewContract;
  }
  ...
  Future forceClosedStatus() async {
    Response response = await _typeService.byCode(ConfigType.prospectStatus);
    if (response.statusCode == 200) {
      List<StbptypeModel> data = [];
      for (var item in response.body) {
      ...
      data.removeWhere((element) => element.sbttypename != 'Force Closed');
      return data.first.sbtid;
    }
    return null;
  }
  ...
  void histories(int prospectid) async {
    Response response = await _prospectService.histories(prospectid);
    if (response.statusCode == 200)
      _historyViewContract.onSuccessFetchHistory(response);
  }
