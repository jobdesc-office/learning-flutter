...
class StBpTypeContactTypePresenter extends CustomGetXController {
  ...
  late IndexViewContract _stBpTypeContactTypeViewContract;
  set stBpTypeContactTypeViewContract(IndexViewContract stBpTypeViewContract) {
    _stBpTypeContactTypeViewContract = stBpTypeViewContract;
  }

  late EditViewContract _stBpTypeContactTypeFetchDataContract;
  set stBpTypeContactTypeFetchDataContract(
      EditViewContract stBpTypeFetchDataContract) {
    _stBpTypeContactTypeFetchDataContract = stBpTypeFetchDataContract;
  }
  ...
  void changeStatus(BuildContext context, int id, bool status) async {
    ...
    if (response.statusCode == 200)
      _stBpTypeContactTypeViewContract.onEditSuccess(response,
          context: context);
    else
      _stBpTypeContactTypeViewContract.onErrorRequest(response);
  }

  Future datatables(BuildContext context) async {
    Response type = await _typeService.byCodeMaster(ConfigType.contactType);
    ...
    _sources.contacttypeid.value = typeid;
    _sources.contacttypetype.value = _sources.type.value.typename!;

    Response response = await _stBpTypeService.datatable(typeid);
    if (response.statusCode == 200)
      _stBpTypeContactTypeViewContract.onLoadDatatables(context, response);
    else
      _stBpTypeContactTypeViewContract.onErrorRequest(response);
  }
  ...
  void save(BuildContext context, Map<String, dynamic> body) async {
    ...
    if (response.statusCode == 200)
      _stBpTypeContactTypeViewContract.onCreateSuccess(response,
          context: context);
    else
      _stBpTypeContactTypeViewContract.onErrorRequest(response);
  }
  ...
  void edit(BuildContext context, int id) async {
    ...
    if (response.statusCode == 200)
      _stBpTypeContactTypeFetchDataContract.onSuccessFetchData(response);
    else
      _stBpTypeContactTypeViewContract.onErrorRequest(response);
  }
      ...
      builder: (context) => ConfirmDialog(
        ...
        onPressed: (_, value) async {
          if (value == ConfirmDialogOption.YES_OPTION) {
            ...
            if (response.statusCode == 200)
              _stBpTypeContactTypeViewContract.onDeleteSuccess(response,
                  context: context);
            else
              _stBpTypeContactTypeViewContract.onErrorRequest(response);
          } else {
