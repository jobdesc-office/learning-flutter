...
class StBpTypeProspectCategoryPresenter extends CustomGetXController {
  ...
  late IndexViewContract _stBpTypeProspectCategoryViewContract;
  set stBpTypeProspectCategoryViewContract(
      IndexViewContract stBpTypeViewContract) {
    _stBpTypeProspectCategoryViewContract = stBpTypeViewContract;
  }

  late EditViewContract _stBpTypeProspectCategoryFetchDataContract;
  set stBpTypeProspectCategoryFetchDataContract(
      EditViewContract stBpTypeFetchDataContract) {
    _stBpTypeProspectCategoryFetchDataContract = stBpTypeFetchDataContract;
  }

  Future datatables(BuildContext context) async {
    Response type =
        await _typeService.byCodeMaster(ConfigType.prospectCategory);
    ...
    if (response.statusCode == 200)
      _stBpTypeProspectCategoryViewContract.onLoadDatatables(context, response);
    else
      _stBpTypeProspectCategoryViewContract.onErrorRequest(response);
  }
  ...
  void changeStatus(BuildContext context, int id, bool status) async {
    ...
    if (response.statusCode == 200)
      _stBpTypeProspectCategoryViewContract.onEditSuccess(response,
          context: context);
    else
      _stBpTypeProspectCategoryViewContract.onErrorRequest(response);
  }
  ...
  void save(BuildContext context, Map<String, dynamic> body) async {
    ...
    if (response.statusCode == 200)
      _stBpTypeProspectCategoryViewContract.onCreateSuccess(response,
          context: context);
    else
      _stBpTypeProspectCategoryViewContract.onErrorRequest(response);
  }
  ...
  void edit(BuildContext context, int id) async {
    ...
    if (response.statusCode == 200)
      _stBpTypeProspectCategoryFetchDataContract.onSuccessFetchData(response);
    else
      _stBpTypeProspectCategoryViewContract.onErrorRequest(response);
  }
      ...
      builder: (context) => ConfirmDialog(
        ...
        onPressed: (_, value) async {
          if (value == ConfirmDialogOption.YES_OPTION) {
            ...
            if (response.statusCode == 200)
              _stBpTypeProspectCategoryViewContract.onDeleteSuccess(response,
                  context: context);
            else
              _stBpTypeProspectCategoryViewContract.onErrorRequest(response);
          } else {
