...
class _TabContactType extends StatelessWidget
    implements IndexViewContract, EditViewContract {
  final BuildContext context;
  final presenter = Get.find<StBpTypeContactTypePresenter>();

  _TabContactType(this.context) {
    presenter.datatables(context);

    presenter.stBpTypeContactTypeViewContract = this;
    presenter.stBpTypeContactTypeFetchDataContract = this;
  }
            ...
            child:
              ...
              if (source.value.isformactcat.value)
                source.value.form(
                    context,
                    presenter,
                    _sources.contacttypeid.value,
                    _sources.contacttypetype.value,
                    'Contact Type'),
              ThemeButtonCreate(
                  prefix: 'Contact Type',
                  onPressed: () {
              ...
              if (_sources.stbpcontacttype.isNotEmpty)
                Container(
                  margin: EdgeInsets.only(top: 10),
                  child: Column(
                    children: _sources.stbpcontacttype.map((e) {
                      int index = _sources.stbpcontacttype.indexOf(e);
                      return BsRow(
  ...
  void onLoadDatatables(BuildContext context, Response response) {
    ...
    _sources.stbpcontacttype.value = stbpraw;
  }
